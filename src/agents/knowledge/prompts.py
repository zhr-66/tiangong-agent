...
EVIDENCE_SUFFICIENCY_PROMPT = """判断当前检索结果是否足以完整回答用户问题。
...
## 判断步骤（必须在 reason 中体现）
1. **拆解用户问题为子需求**：如"病因和常用药"→["病因", "常用药"]
2. **逐项核对每个子需求**：在 reason 中列出每个子需求是否有对应内容
3. **得出结论**：任一子需求未覆盖即 sufficient=false

## 关键规则
- 用户问题中有"和"、"及"、"并"连接的多个维度，必须每个维度都有对应内容才判 sufficient=true
- 特别注意：问"常用药/推荐药/症状/检查"等实体关系，但检索结果只有文本描述没有具体实体名 → missing_aspect=graph_rag
- 特别注意：问"统计/数量/排名/库存"等数据，但检索结果只有文本没有具体数字 → missing_aspect=sql

## missing_aspect 规则
- null：所有子需求都覆盖了
- graph_rag：缺少实体关系信息（常用药、推荐药、症状、检查、并发症、药物相互作用等）
- sql：缺少统计数据/库存/排名等运营数据

## 示例
- 问"高血压的病因和常用药"，结果只有病因没有药品名 → sufficient=false, missing_aspect=graph_rag, reason="子需求[病因]:有; 子需求[常用药]:无,缺药品名"
- 问"阿莫西林的适应症"，结果有适应症 → sufficient=true, missing_aspect=null, reason="子需求[适应症]:有"
- 问"头痛是什么疾病和做什么检查"，结果有疾病和检查 → sufficient=true, missing_aspect=null, reason="子需求[疾病]:有; 子需求[检查]:有"

以 JSON 格式输出：
{{"sufficient": true/false, "missing_aspect": "graph_rag|sql|null", "reason": "逐项核对的详细说明"}}
...
- graph_rag：问实体间关系（疾病和症状/药物/科室/检查的关系），需要多跳推理
- nl2sql：问统计数据、数量、排名、趋势、库存
- multi：需要同时查多个来源才能回答的复杂问题
- prescription：处方审核、用药安全校验

## 路由规则（按优先级）
1. 处方审核/配伍禁忌/两药能否同服 → prescription
2. 统计/数量/排名/库存 → nl2sql
3. 两个或以上疾病合并用药（如"糖尿病合并高血压用什么药"）→ multi
4. 同一疾病问"文档类信息 + 图谱类信息"（如"病因和常用药"、"治疗方式和推荐药"）→ multi
   - 文档类信息：病因、预防、治疗方式、描述、指南
   - 图谱类信息：常用药、推荐药、症状、检查、并发症
   - 只有"文档类 + 图谱类"组合才走 multi，"图谱类 + 图谱类"走 graph_rag
5. 药品问说明书内容（适应症、用法用量、不良反应、禁忌、注意事项）→ doc_rag
6. 单一疾病问常用药/推荐药/检查/症状/并发症/可能疾病 → graph_rag（即使问多个图谱关系也是 graph_rag）
7. 单一疾病问文档/指南内容/病因/预防/治疗方式 → doc_rag

## 关键区分
- "阿莫西林的适应症" → doc_rag（查说明书）
- "高血压的常用药" → graph_rag（查疾病-药物关系）
- "高血压的用药指南" → doc_rag（查文档）
- "高血压的病因和常用药" → multi（病因是文档类 + 常用药是图谱类）
- "高血压的病因和预防" → doc_rag（都是文档类信息）
- "头痛可能是什么疾病需要做什么检查" → graph_rag（都是图谱关系，一次 Cypher 可查）
- "维C银翘片和氨氯地平能一起吃吗" → prescription（两药相互作用）
